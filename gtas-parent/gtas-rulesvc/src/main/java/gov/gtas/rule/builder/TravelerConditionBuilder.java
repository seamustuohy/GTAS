...
public class TravelerConditionBuilder extends EntityConditionBuilder {
	...
	protected void addSpecialConditionsWithoutActualConditions(
			StringBuilder bldr) {
		if(isPassenger()){
			bldr.append(getDrlVariableName()).append(":").append(EntityLookupEnum.Pax).append("()");
		} else if (isCrew()){
	...
	protected void addSpecialConditions(StringBuilder bldr) {
		if(isPassenger()){
			bldr.append(EntityLookupEnum.Pax).append("(id == ").append(getDrlVariableName()).append(".id)");
		} else if (isCrew()){
